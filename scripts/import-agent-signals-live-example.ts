...
const FIXTURE_PATH = "fixtures/chatgpt-agent-microsoft-365-live-example.json";
...
async function main() {
  ...
  process.stdout.write(
    [
      "Agent signal live example imported.",
...
main().catch((error) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : "Agent signal live example import failed."}\n`
  );
  process.exit(1);
});
